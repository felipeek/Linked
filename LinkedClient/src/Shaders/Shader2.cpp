...
Shader2::Shader2(std::string filename)
{
...
void Shader2::useShader()
{
	glUseProgram(shader);
}

void Shader2::stopShader()
{
	glUseProgram(0);
}

void Shader2::getUniformLocations()
{
...
void Shader2::update()
{
	...
	glUniform4fv(uniformTextColor, 1, &color[0]);
	glActiveTexture(GL_TEXTURE0);
	glUniform1i(uniformText, 0);
}

void Shader2::setTextColor(glm::vec4& color)
{
	this->color = color;
}

GLuint Shader2::loadShader(const char* filename, GLuint shaderProgram)
{
...
void Shader2::activateAlphaBlend()
{
	glEnable(GL_BLEND);
...
void Shader2::deactivateAlphaBlend()
{
	glEnable(GL_DEPTH_TEST);
	glDisable(GL_BLEND);
}

GLuint Shader2::getShader()
{
	return this->shader;
}